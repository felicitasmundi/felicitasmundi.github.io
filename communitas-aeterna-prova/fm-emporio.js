...
function empNodo(n, d, doc, dentroSvg){
  /* il testo */
  if(n.nodeType === 3){
  ...
  if(tag === "sc-for"){
    ...
    lista.forEach(function(voce){
      ...
      for(var i = 0; i < n.childNodes.length; i++){
        var c = empNodo(n.childNodes[i], d2, doc, dentroSvg || tag === "svg");
  ...
  if(tag === "sc-if"){
    ...
    for(var j = 0; j < n.childNodes.length; j++){
      var c2 = empNodo(n.childNodes[j], d, doc, dentroSvg || tag === "svg");
  ...
  for(var m = 0; m < n.childNodes.length; m++){
    var c4 = empNodo(n.childNodes[m], d, doc, dentroSvg || tag === "svg");
...
function empDisegna(){
  ...
  for(var i = 0; i < tmp.childNodes.length; i++){
    var n = empNodo(tmp.childNodes[i], d, document);
  ...
  var d1 = empBox.firstElementChild;
  if(d1 && d1.getAttribute && (d1.getAttribute("style") || "").indexOf("grid") > -1){
    empBox.setAttribute("style", d1.getAttribute("style"));
    while(d1.firstChild) empBox.appendChild(d1.firstChild);
    empBox.removeChild(d1);
  }
}
